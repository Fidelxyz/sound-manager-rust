<template>
  <div :class="cx('filter')" v-bind="getColumnPT('filter')">
    <div
      ...
      v-bind="{ ...filterInputProps, ...getColumnPT('filterElementContainer') }"
    >
      <component
        :is="filterElement"
        :field="field"
    ...
    <Button
      ...
      v-bind="{
        ...getColumnPT('pcColumnFilterButton', ptmFilterMenuParams),
        ...filterButtonProps.filter,
      }"
    >
      <template #icon="slotProps">
        <component
          :is="
            filterIconTemplate ||
            (hasFilter() ? 'FilterFillIcon' : 'FilterIcon')
          "
          :class="slotProps.class"
          v-bind="getColumnPT('filterMenuIcon')"
        />
      </template>
    </Button>
    <Button
      ...
      @click="clearFilter()"
      v-bind="{
        ...getColumnPT('pcColumnFilterClearButton', ptmHeaderFilterClearParams),
        ...filterButtonProps.inline.clear,
      }"
    >
      <template #icon="slotProps">
        <component
          :is="filterClearIconTemplate || 'FilterSlashIcon'"
          :class="slotProps.class"
          v-bind="getColumnPT('filterClearIcon')"
        />
      </template>
    </Button>
    <Portal>
      ...
      >
        ...
        >
          ...
          <template v-if="display === 'row'">
            <ul
              :class="cx('filterConstraintList')"
              v-bind="getColumnPT('filterConstraintList')"
            >
              <li
              ...
              <li
                :class="cx('filterConstraintSeparator')"
                v-bind="getColumnPT('filterConstraintSeparator')"
              ></li>
              <li
          ...
          <template v-else>
            <div
              v-if="isShowOperator"
              :class="cx('filterOperator')"
              v-bind="getColumnPT('filterOperator')"
            >
              <Select
            ...
            <div
              :class="cx('filterRuleList')"
              v-bind="getColumnPT('filterRuleList')"
            >
              <div
                v-for="(fieldConstraint, i) of fieldConstraints"
                :key="i"
                :class="cx('filterRule')"
                v-bind="getColumnPT('filterRule')"
              >
                <Select
                  ...
                  >
                    <template #icon="iconProps">
                      <component
                        ...
                        v-bind="getColumnPT('pcFilterRemoveRuleButton')['icon']"
                      />
                    </template>
                  </Button>
                </div>
            ...
            <div
              v-if="isShowAddConstraint"
              v-bind="getColumnPT('filterAddButtonContainer')"
            >
              <Button
                type="button"
              ...
              >
                <template #icon="iconProps">
                  <component
                    ...
                    :class="iconProps.class"
                    v-bind="getColumnPT('pcFilterAddRuleButton')['icon']"
                  />
                </template>
              </Button>
            </div>
            <div
              :class="cx('filterButtonbar')"
              v-bind="getColumnPT('filterButtonbar')"
            >
              <Button
...
export default {
  ...
  methods: {
    ...
    getColumnProp() {
      return this.column.props && this.column.props.pt
        ? this.column.props.pt
        : undefined;
    },
    ptmFilterConstraintOptions(matchMode) {
      return {
        context: {
          highlighted:
            matchMode && this.isRowMatchModeSelected(matchMode.value),
        },
      };
    },
    clearFilter() {
    ...
    findNextItem(item) {
      ...
      if (nextItem)
        return getAttribute(nextItem, "data-pc-section") ===
    ...
    findPrevItem(item) {
      ...
      if (prevItem)
        return getAttribute(prevItem, "data-pc-section") ===
          "filterconstraintseparator"
    ...
    isOutsideClicked(target) {
      return (
        !this.isTargetClicked(target) &&
        this.overlay &&
        !(this.overlay.isSameNode(target) || this.overlay.contains(target))
      );
    },
    isTargetClicked(target) {
      return (
        this.$refs.icon &&
        (this.$refs.icon.$el.isSameNode(target) ||
          this.$refs.icon.$el.contains(target))
      );
    },
      ...
      if (!this.outsideClickListener) {
        this.outsideClickListener = (event) => {
          if (
            this.overlayVisible &&
            !this.selfClick &&
            this.isOutsideClicked(event.target)
          ) {
            this.overlayVisible = false;
          }
  ...
  computed: {
    showMenuButton() {
      return (
        this.showMenu &&
        (this.display === "row" ? this.type !== "boolean" : true)
      );
    },
    overlayId() {
