<template>
  ...
  >
    ...
    >
      ...
      <template
        v-if="$slots.paginatorfirstpagelinkicon"
        #firstpagelinkicon="slotProps"
      >
        <slot name="paginatorfirstpagelinkicon" :class="slotProps.class"></slot>
      </template>
      <template
        v-if="$slots.paginatorprevpagelinkicon"
        #prevpagelinkicon="slotProps"
      >
        <slot name="paginatorprevpagelinkicon" :class="slotProps.class"></slot>
      </template>
      <template
        v-if="$slots.paginatornextpagelinkicon"
        #nextpagelinkicon="slotProps"
      >
        <slot name="paginatornextpagelinkicon" :class="slotProps.class"></slot>
      </template>
      <template
        v-if="$slots.paginatorlastpagelinkicon"
        #lastpagelinkicon="slotProps"
      >
        <slot name="paginatorlastpagelinkicon" :class="slotProps.class"></slot>
      </template>
      <template
        v-if="$slots.paginatorjumptopagedropdownicon"
        #jumptopagedropdownicon="slotProps"
      >
        <slot
      ...
      <template
        v-if="$slots.paginatorrowsperpagedropdownicon"
        #rowsperpagedropdownicon="slotProps"
      >
        <slot
    ...
    <div
      :class="cx('tableContainer')"
      :style="[
        sx('tableContainer'),
        { maxHeight: virtualScrollerDisabled ? scrollHeight : '' },
      ]"
      :data-p="dataP"
      v-bind="ptm('tableContainer')"
    >
      <DTVirtualScroller
        ref="virtualScroller"
    ...
    <div
      ref="resizeHelper"
      :class="cx('columnResizeIndicator')"
      style="display: none"
      v-bind="ptm('columnResizeIndicator')"
    ></div>
    <span
      v-if="reorderableColumns"
      ref="reorderIndicatorUp"
      :class="cx('rowReorderIndicatorUp')"
      style="position: absolute; display: none"
      v-bind="ptm('rowReorderIndicatorUp')"
    >
      <component
        :is="
          $slots.rowreorderindicatorupicon ||
    ...
    <span
      ...
      :class="cx('rowReorderIndicatorDown')"
      style="position: absolute; display: none"
      v-bind="ptm('rowReorderIndicatorDown')"
    >
      <component
        :is="
          $slots.rowreorderindicatordownicon ||
...
export default defineComponent({
  ...
  methods: {
    ...
    onColumnHeaderClick(e) {
      ...
      if (this.columnProp(column, "sortable")) {
        ...
        if (
          ...
          getAttribute(targetNode, "data-pc-section") ===
            "columnheadercontent" ||
          getAttribute(targetNode, "data-pc-section") === "sorticon" ||
          getAttribute(targetNode.parentElement, "data-pc-section") ===
            "sorticon" ||
          getAttribute(
            targetNode.parentElement.parentElement,
            "data-pc-section",
          ) === "sorticon" ||
          (targetNode.closest('[data-p-sortable-column="true"]') &&
            !targetNode.closest('[data-pc-section="columnfilterbutton"]') &&
    ...
    filter(data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (const prop in activeFilters) {
          if (
            Object.prototype.hasOwnProperty.call(activeFilters, prop) &&
            prop !== "global"
          ) {
            localFiltered = true;
      ...
      if (
        filteredValue.length === this.value.length ||
    ...
    onRowClick(e) {
      ...
      if (focusedItem) {
        if (event.target?.getAttribute("data-pc-section") === "rowtoggleicon")
          return;

        const targetRow = event.currentTarget?.closest(
    ...
    onTabKey(event, rowIndex) {
      ...
      if (event.code === "Tab" && rows && rows.length > 0) {
        ...
        if (firstSelectedRow) {
          ...
          if (focusedItem && focusedItem !== firstSelectedRow) {
            focusedItem.tabIndex = "-1";
          }
        } else {
          rows[0].tabIndex = "0";
          if (focusedItem !== rows[0]) {
            rows[rowIndex].tabIndex = "-1";
          }
        }
      }
    },
      ...
      if (rowData && this.selection) {
        if (this.dataKey) {
          return this.d_selectionKeys
            ? this.d_selectionKeys[resolveFieldData(rowData, this.dataKey)] !==
                undefined
            : false;
        }
    ...
    updateSelectionKeys(selection) {
      ...
      if (Array.isArray(selection)) {
        for (const data of selection) {
          this.d_selectionKeys[String(resolveFieldData(data, this.dataKey))] =
            1;
        }
      } else {
        this.d_selectionKeys[
          String(resolveFieldData(selection, this.dataKey))
        ] = 1;
      }
    },
    updateEditingRowKeys(editingRows) {
      if (editingRows?.length) {
        ...
        for (const data of editingRows) {
          this.d_editingRowKeys[String(resolveFieldData(data, this.dataKey))] =
            1;
        }
      } else {
    ...
    exportCSV(options, data) {
      ...
      if (!data) {
        ...
        if (options?.selectionOnly) data = this.selection || [];
    ...
    onColumnResizeStart(event) {
      const containerLeft = getOffset(this.$el).left;
    ...
    onColumnResizeEnd() {
      const delta = isRTL(this.$el)
        ? this.lastResizeHelperX - this.$refs.resizeHelper.offsetLeft
      ...
      if (columnWidth + delta > Number.parseInt(minWidth, 10)) {
        if (this.columnResizeMode === "fit") {
          ...
          if (!this.virtualScrollerDisabled) {
            const body = this.$refs.bodyRef?.$el;
            const frozenBody = this.$refs.frozenBodyRef?.$el;
    ...
    resizeTableCells(newColumnWidth, nextColumnWidth) {
      const colIndex = getIndex(this.resizeColumnElement);
      const widths = [];
      const headers = find(
        this.$refs.table,
      ...
      for (const header of headers) {
        widths.push(getOuterWidth(header));
      }
      ...
      let innerHTML = "";
      const selector = `[data-pc-name="datatable"][${this.$attrSelector}] > [data-pc-section="tablecontainer"] ${this.virtualScrollerDisabled ? "" : '> [data-pc-name="virtualscroller"]'} > table[data-pc-section="table"]`;
    ...
    onColumnHeaderMouseDown(e) {
      ...
      if (
        ...
        if (
          ...
          getAttribute(event.target, '[data-pc-section="columnresizer"]')
        )
    ...
    onColumnHeaderDragStart(e) {
      ...
      this.colReorderIconWidth = getHiddenElementOuterWidth(
        this.$refs.reorderIndicatorUp,
      );
      this.colReorderIconHeight = getHiddenElementOuterHeight(
        this.$refs.reorderIndicatorUp,
      );
    ...
    onColumnHeaderDragOver(e) {
      ...
      if (
        ...
        event.preventDefault();
        const containerOffset = getOffset(this.$el);
        ...
        if (this.draggedColumnElement !== dropHeader) {
          ...
          this.$refs.reorderIndicatorDown.style.top = `${
            dropHeaderOffset.top - containerOffset.top + dropHeader.offsetHeight
          }px`;
    ...
    onColumnHeaderDrop(e) {
      ...
      if (this.draggedColumnElement) {
        const dragIndex = getIndex(this.draggedColumnElement);
        const dropIndex = getIndex(this.findParentHeader(event.target));
        ...
        if (allowDrop) {
          const isSameColumn = (col1, col2) =>
            this.columnProp(col1, "columnKey") ||
            this.columnProp(col2, "columnKey")
              ? this.columnProp(col1, "columnKey") ===
                this.columnProp(col2, "columnKey")
              : this.columnProp(col1, "field") ===
                this.columnProp(col2, "field");
          const dragColIndex = this.columns.findIndex((child) =>
            isSameColumn(child, this.draggedColumn),
          );
          let dropColIndex = this.columns.findIndex((child) =>
            isSameColumn(child, column),
          );
          const widths = [];
          const headers = find(
            this.$el,
          ...
          const remainingItems = widths.filter(
            (_, index) => index !== dragColIndex,
          );
    ...
    onRowDragOver(e) {
      ...
      if (this.rowDragging && this.draggedRowIndex !== index) {
        ...
        if (pageY < rowMidY) {
          ...
          if (prevRowElement) {
            prevRowElement.setAttribute(
              "data-p-datatable-dragpoint-bottom",
              "true",
            );
            !this.isUnstyled &&
              addClass(prevRowElement, "p-datatable-dragpoint-bottom");
          } else {
            rowElement.setAttribute("data-p-datatable-dragpoint-top", "true");
            !this.isUnstyled &&
              addClass(rowElement, "p-datatable-dragpoint-top");
          }
        } else {
          if (prevRowElement) {
            prevRowElement.setAttribute(
    ...
    toggleRow(event) {
      ...
      expanded
        ? this.$emit("row-expand", rest)
        : this.$emit("row-collapse", rest);
    },
    toggleRowGroup(e) {
      const event = e.originalEvent;
      const data = e.data;
    ...
    isRowGroupExpanded(rowData) {
      if (this.expandableRowGroups && this.expandedRowGroups) {
        const groupFieldValue = resolveFieldData(rowData, this.groupRowsBy);
    ...
    saveState() {
      ...
      if (this.d_sortField) {
        ...
        if (typeof this.d_sortField !== "function")
    ...
    restoreState() {
      ...
      if (
        typeof parsedState.expandedRows === "object" &&
        parsedState.expandedRows !== null
      ) {
      ...
      if (
        typeof parsedState.selection === "object" &&
        parsedState.selection !== null
      ) {
        if (
          typeof parsedState.selectionKeys === "object" &&
          parsedState.selectionKeys !== null
        ) {
  ...
  computed: {
    ...
    headerColumnGroup() {
      return this.columnGroups?.find(
        (group) => this.columnProp(group, "type") === "header",
      );
    },
    footerColumnGroup() {
      return this.columnGroups?.find(
        (group) => this.columnProp(group, "type") === "footer",
      );
    },
    hasFilters() {
      return (
