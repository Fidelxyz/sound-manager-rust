<template>
  ...
  >
    ...
    ></span>
    <div
      :class="cx('columnHeaderContent')"
      v-bind="getColumnPT('columnHeaderContent')"
    >
      <component
        v-if="column.children && column.children.header"
        :is="column.children.header"
        :column="column"
      />
      <span
        v-if="columnProp('header')"
        :class="cx('columnTitle')"
        v-bind="getColumnPT('columnTitle')"
        >{{ columnProp("header") }}</span
      >
      <span v-if="columnProp('sortable')" v-bind="getColumnPT('sort')">
        <component
          :is="
            (column.children && column.children.sorticon) || sortableColumnIcon
          "
      ...
      <Badge
        v-if="isMultiSorted()"
        :class="cx('pcSortBadge')"
        :pt="getColumnPT('pcSortBadge')"
        :value="getBadgeValue()"
        size="small"
      />
      <DTHeaderCheckbox
        v-if="
          columnProp('selectionMode') === 'multiple' && filterDisplay !== 'row'
        "
        :checked="allRowsSelected"
        @change="onHeaderCheckboxChange"
        :disabled="empty"
        :headerCheckboxIconTemplate="
          column.children && column.children.headercheckboxicon
        "
        :column="column"
        :unstyled="unstyled"
        :pt="pt"
      ...
      <DTColumnFilter
        v-if="
          filterDisplay === 'menu' && column.children && column.children.filter
        "
        ...
        :filterAddIconTemplate="
          column.children && column.children.filteraddicon
        "
        :filterRemoveIconTemplate="
          column.children && column.children.filterremoveicon
        "
        :filterClearIconTemplate="
          column.children && column.children.filterclearicon
        "
        :filters="filters"
  ...
  methods: {
    ...
    getColumnPT(key) {
      const columnMetaData = {
        ...
        context: {
          index: this.index,
          sortable:
            this.columnProp("sortable") === "" || this.columnProp("sortable"),
          sorted: this.isColumnSorted(),
          resizable: this.resizableColumns,
          size: this.$parentInstance?.$parentInstance?.size,
          showGridlines:
            this.$parentInstance?.$parentInstance?.showGridlines || false,
        },
      };
    ...
    onKeyDown(event) {
      if (
        ...
        getAttribute(event.currentTarget, "data-p-sortable-column")
      ) {
    ...
    getMultiSortMetaIndex() {
      return this.multiSortMeta.findIndex(
        (meta) =>
          meta.field === this.columnProp("field") ||
          meta.field === this.columnProp("sortField"),
      );
    },
    getBadgeValue() {
    ...
    updateStickyPosition() {
      if (this.columnProp("frozen")) {
        ...
        if (align === "right") {
          let pos = 0;
          let next = getNextElementSibling(
            this.$el,
          ...
          if (next) {
            pos =
              getOuterWidth(next) +
              parseFloat(next.style["inset-inline-end"] || 0);
          }

          this.styleObject.insetInlineEnd = pos + "px";
        } else {
          let pos = 0;
          let prev = getPreviousElementSibling(
            this.$el,
            '[data-p-frozen-column="true"]',
          );
          ...
          if (prev) {
            pos =
              getOuterWidth(prev) +
              parseFloat(prev.style["inset-inline-start"] || 0);
          }
        ...
        if (filterRow) {
          ...
          if (filterRow.children[index]) {
            filterRow.children[index].style["inset-inline-start"] =
              this.styleObject["inset-inline-start"];
            filterRow.children[index].style["inset-inline-end"] =
              this.styleObject["inset-inline-end"];
          }
        }
      }
    },
