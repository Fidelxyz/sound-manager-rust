<template>
  <tfoot
    ...
    v-bind="
      columnGroup
        ? { ...ptm('tfoot', ptmTFootOptions), ...getColumnGroupPT('root') }
        : ptm('tfoot', ptmTFootOptions)
    "
  ...
  >
    ...
    <template v-else>
      <tr
        v-for="(row, i) of getFooterRows()"
        :key="i"
        role="row"
        v-bind="{ ...ptm('footerRow'), ...getRowPT(row, 'root', i) }"
      >
        <template
          v-for="(col, j) of getFooterColumns(row)"
...
export default {
  ...
  methods: {
    ...
    getColumnGroupPT(key) {
      ...
      return mergeProps(
        this.ptm(`columnGroup.${key}`, { columnGroup: columnGroupMetaData }),
        this.ptm(`columnGroup.${key}`, columnGroupMetaData),
        this.ptmo(this.getColumnGroupProps(), key, columnGroupMetaData),
      );
    },
    getColumnGroupProps() {
      return this.columnGroup &&
        this.columnGroup.props &&
        this.columnGroup.props.pt
        ? this.columnGroup.props.pt
        : undefined; //@todo
    },
    getRowPT(row, key, index) {
      ...
      return mergeProps(
        this.ptm(`row.${key}`, { row: rowMetaData }),
        this.ptm(`row.${key}`, rowMetaData),
    ...
    getFooterRows() {
      return this.d_footerRows?.get(
        this.columnGroup,
        this.columnGroup.children,
      );
    },
